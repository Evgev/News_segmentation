...
# Загрузка данных
def load_data():
    source_data = pd.read_excel("/root/News_segmentation/data/lenta_ru_2024_03_19-2025_03_18_without_bodies.xlsx")
    return source_data
...
def train_word2vec(texts):
    ...
    model = Word2Vec(sentences=tokenized_texts, vector_size=200, window=5, min_count=5, workers=8,  epochs=100)
    return model
...
def main():
    ...
    texts = source_data['titles'].dropna().tolist()
    
    # Обучение Word2Vec
    model = train_word2vec(texts)
    
    # Пример категорий
    categories = ["экономика", "политика", "сми", "технологии", "культура", "спорт", "война"]
